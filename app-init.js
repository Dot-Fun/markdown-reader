// Bootstrap wiring for dotfun Markdown Studio with namespace-aware guards.

(function bootstrapMarkdownStudio() {
  ...
  function dependenciesReady() {
    if (!namespace.baseReady || !namespace.runtimeReady) {
      return false;
    }
    if (!requiredFunctions.every((key) => typeof namespace[key] === 'function')) {
      return false;
    }
    if (!requiredRefs.every((key) => key in namespace && namespace[key])) {
      return false;
    }
    return 'previewSettings' in namespace;
  }
  ...
  function start() {
    ...
    const btnCopyDocs = namespace.btnCopyDocs || null;
    const dismissBookmark = namespace.dismissBookmark || null;

    loadPreviewSettings();
    hydrateTheme();
    applyPreviewSettings(namespace.previewSettings);
    hydrateEditor();
    initializeSettingsUI();
    ...
    if (btnCopyDocs && typeof copyPreviewForDocs === 'function') {
